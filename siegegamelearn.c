...
void move_player(Player* p) {
  ...
  p->x += DIR_X[p->dir];
  p->y += DIR_Y[p->dir];
  if((readcharxy(p->x, p->y) & 0x7f) != ' ') p->collided = 1;
  draw_player(p);
}
...
void human_control(Player *p) {
  ...
  joy = joy_read(1); // read first joystick
...
byte ai_try_dir(Player *p, dir_t dir, byte shift) {
  ...
  if(x < COLS && y < ROWS && (readcharxy(x, y) & 0x7f) == ' ') {
    p->dir = dir;
    return 1;
  }
  else {
...
void play_round() {
  ...
  flash_colliders();
  // add scores to players taht didn't collide
  if (players[0].collided) players[1].score++;
  if (players[1].collided) players[0].score++;
  // increase speed
  if (frames_per_move > MAX_SPEED) frames_per_move--;
  // game over?
  if (players[0].score >= MAX_SCORE) declare_winner(0);
  else if (players[1].score >= MAX_SCORE) declare_winner(1);
}
